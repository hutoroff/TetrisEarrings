...
| — | J1 | TC2030-IDC-NL footprint | Tag-Connect pads | SWD; bare copper, nothing to buy |
| — | J2 | Dock contacts | 2 pads | 5 V charging; bare copper, nothing to buy |
...
Total: 104 purchased components (24 BOM lines) plus 2 copper-pad footprints. Machine-readable list: [hardware/BOM.csv](hardware/BOM.csv).

## Electronics (Altium)

Schematic, PCB and BOM live in [`hardware/`](hardware/) as a KiCad project (text, generated by `gen_schematic.py` / `gen_pcb.py`). The PCB is 24 × 48 mm, 4 layers: the LEDs are on the front, everything else is on the back, and the inner layers are GND and VSW planes. It is fully routed, with DRC 0 errors and 0 differences from the schematic. Ready-to-upload JLCPCB files are in [`hardware/fab/`](hardware/fab/).

Altium Designer 21+ opens it directly: **File → Import Wizard → KiCad Design Files** → select the `hardware/` folder → Finish → **Project → Compile**. Circuit description, layout notes and tuning knobs (battery cutoff, charge current, battery size) are in [hardware/README.md](hardware/README.md).
